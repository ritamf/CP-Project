                  ...
                  pixel_t * h_odata)
{
    ...
    #pragma omp parallel for shared(h_odata, h_idata) private(i, j) firstprivate(w, h) schedule(guided,8)
    for(i=0; i<h; i++) //height image
    {
        for(j=0; j<w; j++) //width image
        {   
    ...
    #pragma omp parallel for shared(h_odata, h_idata) private(j, sumIx2, sumIy2, sumIxIy, Ix, Iy) firstprivate(w, h, ws, threshold) schedule(guided,8)
    for(i=ws+1; i<h-ws-1; i++) //height image
    {
        for(j=ws+1; j<w-ws-1; j++) //width image
        {
