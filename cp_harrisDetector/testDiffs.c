...
int main( int argc, char** argv) 
{
	...
	unsigned char* h_idata1=NULL;
	unsigned int h1,w1;
	//load pgm
	if (sdkLoadPGM<unsigned char>(argv[optind], &h_idata1, &w1, &h1) != true) {
		printf("Failed to load image file: %s\n", argv[1]);
		exit(1);
	}

	// allocate host memory 2
	unsigned char* h_idata2=NULL;
	unsigned int h2,w2;
	//load pgm
	if (sdkLoadPGM<unsigned char>(argv[optind+1], &h_idata2, &w2, &h2) != true) {
		printf("Failed to load image file: %s\n", argv[2]);
		exit(1);
	}
