...
for i, string in enumerate(results):
    if string.endswith(".pgm\n"):
        ...
        valueCuda = float(results[i+2].split("Device processing time: ")[1].split(" (ms)")[0])
        efficiency = (valueHost - valueCuda) / valueHost * 100
        resDict[string[:-1]] = {"valueHost":valueHost, "valueCuda":valueCuda, "efficiency" : f"{efficiency:.{2}f}\%"}
